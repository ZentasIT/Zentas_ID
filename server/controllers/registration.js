...
function generateAccessToken(user) {
    return jwt.sign({ userId: user.UserId }, process.env.JWT_SECRET, { expiresIn: '24h' });
}
...
class UserController{
    async registration(req, res, next){
        try{
            ...
            const { name, family, otchestvo, rdate, number, email } = req.body; // Получение данных и запроса
            const activationToken = jwt.sign({ email }, process.env.JWT_SECRET, { expiresIn: '45m' });
            const newUser = { UserId: generateUUID(16), DislayId: generateNumUUID(8), firstName: name, lastName: family, middleName: otchestvo, Date: rdate, phone: number, email: email, activationLink: activationToken, isActivated: false };
            const userFolderName = newUser.UserId; // ID для создания папки
            const newUserFolderPath = path.join(__dirname, '..', 'database', 'uFolders', userFolderName); // Путь создания папки
            ...
            try{
                ...
                console.log('Ошибка чтения файла users', err);
            }
    ...
    async activate(req, res, next) {
        try {
            ...
            if (!user) return res.status(400).json({ message: 'Ссылка не найдена' });
            user.isActivated = true;
            fs.writeFileSync(filePath, JSON.stringify(users, null, 4));
            res.redirect('https://id.zentas.ru/auth#verify');
        } catch (error) {
            next(error);
        }
    }
    ...
    async login(req, res, next){
        try{
            if (((req.body && req.body.email) || (req.body && req.body.user_email))) {
                const {email, user_email, user_code} = req.body;
                ...
                if (email && email !== "") {
                    ...
                    const authCode = Math.floor(100000 + Math.random() * 900000); // Генерация временного кода
                    const mailOptions = {
                        from: '"Зентас ID" <info@zentas.ru>', to: email, subject: 'Код для авторизации', html: `
