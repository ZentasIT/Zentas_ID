...
class UserController{
    async registration(req, res, next) {
        try {
            ...
            console.log("Received data:", req.body); // Добавим эту строку для отладки

            // Хешируем пароль
            const hashedPassword = await bcrypt.hash(password, 10);

            // Генерируем активационную ссылку с помощью JWT токена, который будет содержать информацию для подтверждения
            ...
            const transporter = nodemailer.createTransport({
                host: 'smtp.zentas.ru',
                port: 25, // порт вашего SMTP сервера
                secure: false, // true для использования SSL/TLS, false для обычного текстового соединения
                auth: {
                    user: 'info@zentas.ru', // ваша учетная запись электронной почты
                    pass: 'AAAlesha11223' // ваш пароль электронной почты
                },
                tls: {
                    rejectUnauthorized: false // разрешить использование самоподписанных сертификатов
                }
            });


            const mailOptions = {
                from: '"Зентас ID" <info@zentas.ru>', // Здесь указывается имя и адрес отправителя
                to: email,
...
`
            ...
            res.status(201).json({ success: 'User registered successfully. Please check your email to activate your account.' });
        } catch (error) {
            console.error("Error during registration:", error); // Добавим эту строку для отладки
            next(error);
        }
    }
    ...
    async activate(req, res, next) {
        try {
            ...
            // Расшифровываем токен активации
            const decodedToken = jwt.verify(activationToken, process.env.JWT_SECRET);

            // Проверяем, что токен содержит правильную информацию
            if (!decodedToken.email) {
                return res.status(400).json({ message: 'Invalid activation link.' });
            }

            // Ищем пользователя в базе данных по электронной почте из токена
            ...
            res.redirect('http://localhost:5000/auth#verefy'); // Редирект на страницу входа после активации
        } catch (error) {
            next(error);
        }
    }
    async login(req, res, next) {
        ...
        try {
            if (req.body.email && req.body.email !== "") {
                ...
                // Отправляем уведомление о успешной аутентификации с IP-адресом


                return res.status(200).json({ success: 'Authentication code sent to email.' });
            } else if (req.body.user_email && req.body.user_code) {
                const { user_email, user_code } = req.body;
                ...
                if (user.tempCode !== user_code) {
                    return res.status(200).json({ message: 'Invalid authentication code.' });
                }
                ...
                await user.save();
                res.cookie('refreshToken', refreshToken, { httpOnly: true, maxAge: 30 * 24 * 60 * 60 * 1000 }); // 30 дней в миллисекундах
                res.cookie('accessToken', accessToken, { maxAge: 900000 }); // 15 минут в миллисекундах

                // Отправка ответа клиенту
                res.json({ success: 'Authentication successful' });
                // return res.json({ accessToken, refreshToken });
                const ipAddress = req.ip;
                const authNotificationOptions = {
                    from: '"Зентас ID" <info@zentas.ru>',
                    to: email,
                    subject: 'Successful Authentication',
                    text: `You have successfully logged in. IP address: ${ipAddress}`
                };
                await transporter.sendMail(authNotificationOptions);
            } else {
        ...
        async refresh(req, res, next) {
            try {
                ...
                // Проверка наличия пользователя с таким рефреш токеном
                if (!user) {
                    return res.status(403).json({ message: 'Refresh token not found' });
                }

                // Верификация рефреш токена
    ...
    async logout(req, res, next) {
        try {
            ...
            // Декодируем accessToken, чтобы получить userId
            ...
            // Удаление refreshToken у пользователя
